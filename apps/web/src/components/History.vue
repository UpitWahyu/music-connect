<script setup lang="ts">
import { onMounted, ref } from "vue";
import { api, type HistoryDTO } from "../lib/api";
import { store, refreshState } from "../composables/useMusic";

const history = ref<HistoryDTO[]>([]);

async function load(): Promise<void> {
  history.value = (await api.history()).history;
}

async function playTrack(trackId: string): Promise<void> {
  if (!store.selectedDevice) return;
  await api.play(store.selectedDevice, trackId);
  await refreshState();
}

onMounted(load);
</script>

<template>
  <section class="rounded-2xl border border-gray-700 bg-gray-800 p-4">
    <h2 class="mb-2 text-sm font-semibold text-gray-300">🕘 Riwayat Putar</h2>
    <ul v-if="history.length" class="divide-y divide-gray-800">
      <li v-for="h in history" :key="h.id" class="flex items-center justify-between gap-2 py-2 text-sm">
        <div class="min-w-0 flex-1">
          <div class="truncate font-medium">{{ h.title }}</div>
          <div class="truncate text-xs text-gray-500">
            {{ h.artist }} · {{ new Date(h.playedAt).toLocaleString("id-ID", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }) }}
          </div>
        </div>
        <button class="rounded bg-green-600/80 px-2 py-1 text-xs hover:bg-green-500" @click="playTrack(h.trackId)">▶</button>
      </li>
    </ul>
    <p v-else class="py-3 text-center text-sm text-gray-500">Belum ada riwayat — mulai putar sesuatu.</p>
  </section>
</template>
